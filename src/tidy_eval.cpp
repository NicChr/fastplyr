...
cpp11::list cpp_quos_drop_null(cpp11::list quos){
  ...
  for (int i = 0; i < quos.size(); ++i){
    if (!Rf_isNull(rlang::quo_get_expr(quos[i]))){
...
SEXP cpp_grouped_eval_tidy(SEXP data, SEXP quos, bool recycle){
  ...
  for (int i = 0; i < n_groups; ++i){
    ...
    for (int m = 0; m < n_quos; ++m){
      ...
      recycled_size = (recycled_size == 0 || result_size == 0) ? 0 : std::max(recycled_size, result_size);
    ...
    if (recycle){
      for (int j = 0; j < n_quos; ++j){
        SET_VECTOR_ELT(inner_container, j, cheapr::rep_len(VECTOR_ELT(inner_container, j), recycled_size));
      }
    }
  ...
  SEXP groups_container;
  PROTECT_INDEX groups_container_idx;
  ...
  SEXP result_container = Rf_protect(Rf_allocVector(VECSXP, 1)); ++NP;
...
//     }
//     SET_VECTOR_ELT(results, m, intermediate_container);
//   }
//   Rf_unprotect(NP);
//   return results;
// }
...
//   SEXP top_env = Rf_protect(get_mask_top_env(mask)); ++NP;
//
//   SEXP chunk_locs, chunk, result, inner_container, r_recycled_size;
//
//   PROTECT_INDEX chunk_locs_idx, chunk_idx,
//   result_idx, inner_container_idx, r_recycled_size_idx;
//
//   R_ProtectWithIndex(chunk_locs = R_NilValue, &chunk_locs_idx); ++NP;
//   R_ProtectWithIndex(chunk = data_subset, &chunk_idx); ++NP;
//   R_ProtectWithIndex(result = R_NilValue, &result_idx); ++NP;
//   R_ProtectWithIndex(inner_container = R_NilValue, &inner_container_idx); ++NP;
//   R_ProtectWithIndex(r_recycled_size = R_NilValue, &r_recycled_size_idx); ++NP;
//
//
//   // outer container will contain lists of expressions for each group
//   // group container will contain the rows of each distinct group
//   SEXP outer_container = Rf_protect(Rf_allocVector(VECSXP, n_groups)); ++NP;
//
//   // These sizes tell us how many times to repeat each group for each expression
//   // if recycle is true then all expressions are recycled per-group and
//   // so the amount to repeat each group is constant across all expressions.
//   // To be consistent and to make code cleaner later,
//   // we allocate a length `n_quos` list,
//   // but when recycle is true we just move the same recycled_sizes int vec
//   // to each element (they all point to the same memory)
//   // and overwriting one element of any int vec affects the others
//   //
//
//   SEXP recycled_sizes;
//   PROTECT_INDEX recycled_sizes_idx;
...
//
//   SEXP recycled_sizes_container = Rf_protect(Rf_allocVector(VECSXP, n_quos)); ++NP;
//   Rf_setAttrib(recycled_sizes_container, R_NamesSymbol, quo_names);
//   const SEXP *p_recycled_sizes_container = VECTOR_PTR_RO(recycled_sizes_container);
//   if (recycle){
//     R_Reprotect(recycled_sizes = Rf_allocVector(INTSXP, n_groups), recycled_sizes_idx);
//     for (int m = 0; m < n_quos; ++m){
...
//
//   for (int i = 0; i < n_groups; ++i){
//     // Initialise it to something less than 0 for recycling logic to work
//     recycled_size = -1;
//
//     if (has_groups){
//       R_Reprotect(chunk_locs = p_rows[i], chunk_locs_idx);
...
//       SET_VECTOR_ELT(inner_container, m, result);
//       recycled_size = (recycled_size == 0 || result_size == 0) ? 0 : std::max(recycled_size, result_size);
//       recycled_sizes = p_recycled_sizes_container[m];
//       INTEGER(recycled_sizes)[i] = recycled_size;
//     }
//     if (recycle){
//       R_Reprotect(r_recycled_size = Rf_ScalarInteger(recycled_size), r_recycled_size_idx);
//       R_Reprotect(inner_container = cheapr::recycle(inner_container, r_recycled_size), inner_container_idx);
//     }
//     SET_VECTOR_ELT(outer_container, i, inner_container);
//   }
//   // Combine results
...
//   }
//
//   // Concatenate recycled size
//
//   intermediate_container = Rf_protect(Rf_allocVector(VECSXP, 2)); ++NP;
//   SET_VECTOR_ELT(intermediate_container, 0, recycled_sizes_container);
//   SET_VECTOR_ELT(intermediate_container, 1, results);
//
//   Rf_unprotect(NP);
...
SEXP cpp_grouped_eval_tidy0(SEXP data, SEXP quos, bool recycle){
  ...
  // group container will contain the rows of each distinct group
  SEXP outer_container = Rf_protect(Rf_allocVector(VECSXP, n_groups)); ++NP;
  SEXP recycled_sizes = Rf_protect(Rf_allocVector(INTSXP, n_groups)); ++NP;
  int *p_recycled_sizes = INTEGER(recycled_sizes);
  int recycled_size = -1;
  int result_size;

  for (int i = 0; i < n_groups; ++i) p_recycled_sizes[i] = NA_INTEGER;
  ...
  for (int i = 0; i < n_groups; ++i){
    ...
    p_recycled_sizes[i] = recycled_size;
    SET_VECTOR_ELT(outer_container, i, inner_container);
  }
  // Combine results
  SEXP results = Rf_protect(Rf_allocVector(VECSXP, n_quos)); ++NP;
  ...
  intermediate_container = Rf_protect(Rf_allocVector(VECSXP, 2)); ++NP;
  SEXP recycled_sizes_container = Rf_protect(Rf_allocVector(VECSXP, 1)); ++NP;
  Rf_setAttrib(recycled_sizes_container, R_NamesSymbol, Rf_mkString("group_sizes"));
  SET_VECTOR_ELT(recycled_sizes_container, 0, recycled_sizes);
  SET_VECTOR_ELT(intermediate_container, 0, recycled_sizes_container);
  SET_VECTOR_ELT(intermediate_container, 1, results);

  Rf_protect(results = cheapr::list_c(intermediate_container)); ++NP;
