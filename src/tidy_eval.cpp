...
SEXP cpp_eval_all_tidy(SEXP quos, SEXP mask){
  ...
  for (int i = 0; i < n_exprs; ++i){
    ...
    SEXP result = Rf_protect(rlang::eval_tidy(quo, mask, R_NilValue)); ++NP;
...
//   SEXP env = Rf_en new.env(hash = FALSE, parent = emptyenv())
...
//   for (int i = 0; i < n_exprs; ++i){
//     SEXP quo = Rf_protect(VECTOR_ELT(quos, i)); ++NP;
//     SEXP expr = Rf_protect(rlang::quo_get_expr(quo)); ++NP;
//     SEXP expr_name = Rf_protect(STRING_ELT(expr_names, i)); ++NP;
//     SEXP rho = Rf_protect(rlang::quo_get_env(quo)); ++NP;
//     SEXP sym = Rf_protect(Rf_installChar(expr_name)); ++NP;
//     SEXP result = Rf_protect(rlang::eval_tidy(expr, mask, rho));
//     Rf_defineVar(sym, result, mask);
//     if (!Rf_isNull(result)){
...
//     }
//     if (TYPEOF(branch) == SYMSXP &&
//         STRING_ELT(cpp_fun_ns(rlang::sym_as_character(branch), rho), 0) == ns_str){
//       out = true;
//       break;
//     }
//     // current = CDR(current);
