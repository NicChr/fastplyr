...
|                | ### dplyr         | ### fastplyr             |
+================+===================+==========================+
| `.by`          | Groups are sorted | Groups are always sorted |
|                | by order of first | by default, even when    |
|                | appearance always | using `.by`. One can use |
|                | when using `.by`  | the other by setting     |
|                |                   | `.order = FALSE`         |
+----------------+-------------------+--------------------------+
| Many groups    | Generally slow    | Designed to be fast for  |
|                | for data with     | data with many groups.   |
|                | many groups.      |                          |
+----------------+-------------------+--------------------------+
| Handling of    | Executes          | Some expressions are     |
| `...`          | expressions in a  | executed independently   |
| expressions    | way that latter   | to each other            |
|                | expressions       |                          |
|                | depend on prior   |                          |
|                | ones              |                          |
+----------------+-------------------+--------------------------+
| Optimisations  | Expressions are   | Many functions are       |
|                | run by-group with | optimised to either      |
|                | minimal overhead, | ignore groups or use     |
|                | slow for many     | faster methods           |
|                | groups            |                          |
+----------------+-------------------+--------------------------+
| Duplicate rows | No dedicated      | Dedicated function       |
|                | function for      | `f_duplicates` can do    |
|                | this, solution    | this very fast and with  |
|                | using             | fine control.            |
|                | `group_ by | >    |                          |
|                |  filter(n() > 1)` |                          |
|                | are generally     |                          |
...
|                | data.             |                          |
+----------------+-------------------+--------------------------+
| Row slicing    | `slice()`         | Data-masked expressions  |
|                | supports          | not supported in         |
|                | data-masked       | `f_slice_` functions.    |
|                | expressions       | Use `f_filter()` for     |
|                | supplied to `...` | this behaviour.          |
+----------------+-------------------+--------------------------+
| Memory usage   | High memory usage | Lower usage compared to  |
|                |                   | dplyr                    |
+----------------+-------------------+--------------------------+
| joins          | Accepts different | Accepts only equality    |
|                | types of joins,   | joins of the form        |
|                | e.g. rolling and  | `x == y`                 |
|                | equality joins.   |                          |
+----------------+-------------------+--------------------------+
| rowwise        | `rowwise_df`      | `rowwise_df` not         |
|                | accepted and      | accepted, must use       |
|                | everything        | `f_rowwise_df` which     |
|                | sub-setted        | creates a `grouped_df`   |
|                | implicitly using  | with a row ID col.       |
|                | `[[`              | Implicit `[[` subsetting |
|                |                   | does not occur.          |
+----------------+-------------------+--------------------------+
