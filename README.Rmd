...
| `...`          | expressions in a   | executed independently to |
| expressions    | way that latter    | each other                |
|                | expressions depend |                           |
|                | on prior ones      |                           |
+----------------+--------------------+---------------------------+
...
|                | function for this, | `f_duplicates` can do     |
|                | solution using     | this very fast and with   |
|                | `group_ by |       | fine control.             |
|                | > filter(n() > 1)` |                           |
|                | are generally slow |                           |
|                | for larger data.   |                           |
+----------------+--------------------+---------------------------+
| Row slicing    | `slice()` supports | Data-masked expressions   |
|                | data-masked        | not supported in          |
|                | expressions        | `f_slice_` functions. Use |
|                | supplied to `...`  | `f_filter()` for this     |
|                |                    | behaviour.                |
+----------------+--------------------+---------------------------+
